...
### Admin-dashboard Phase D: employee check-in history view, COMPLETE (2026-05-23) ✅

The employee's own check-in history, the transparency counterweight to the Phase B aggregate read API (Phase B is now the next priority). Lowest regulatory risk in the plan because the data subject IS the requestor — but the round-1 plan-eng-critic still surfaced a real Article 5(1)(c) data-minimisation issue in the source draft that the revision closes.

- [x] `GET /api/me/check-ins?limit=N` in `server/index.js`. Behind `requireAuth`, scoped strictly by `req.auth.userId`. Module-scoped `MAX_LIMIT = 200`, default 50, clamp-never-400 (matches `/api/me` PATCH tolerance). Single parameterised SELECT with **explicit per-kind JSONB headline extraction** via `CASE kind WHEN 'breath' THEN payload->'personalReport'->>'headline' WHEN 'motion' THEN payload->'neuroReport'->>'headline' WHEN 'heart' THEN payload->'heartReport'->>'headline' END`. Response shape `{ checkIns: [{id,kind,createdAt,headline}], limit, truncated }` — raw `payload` never returned. Servicing index is the existing `check_ins_user_created_idx (user_id, created_at DESC)` from 002_schema.sql.
- [x] `server/test-me-check-ins.js`: 8 `node --test` integration cases against real PG mirroring `test-admin-endpoints.js`: auth (401), empty, allowlist key-set, **negative-assertion stringify** (asserts the response JSON contains none of the raw payload sub-keys or classification tokens — anchored on `:` suffix to avoid false positives on the `kind:'heart'` discriminator), per-kind headline, cross-user isolation (across orgs AND same-org), limit clamping (10000, -1, 'abc', 0, missing), order newest-first.
- [x] `client/src/views/HistoryView.jsx`: own `hsv-` css prefix + `cssInjected/useCss()` pattern. 4-state machine (loading / empty / list / error) plus `errorKind: auth-expired | network`. Loading reuses the wave-bar pattern. **Empty state** is copy-drafted with the explicit transparency framing ("Nobody else can see this list, including your employer") + two CTAs back to Breath/Heart capture. List renders instrument-card-style rows with monospace tabular-nums time + brass/bone/warn modality chip + ellipsis-truncated headline, day-grouped (Today/Yesterday/This week/Earlier) in local TZ with `<h2>` labels and semantic `<ol>/<li>`. Sr-only `<time dateTime>` on each row. Footer "Showing your N most recent check-ins" only when `truncated`. Row tap-to-expand is **explicitly deferred** (list-only this phase).
- [x] `ParticipantView.jsx`: new `'history'` stage, mounts HistoryView with `onBack` returning to `results`/`blow`, plus `onBreath` and `onHeart` for the empty-state CTAs. `r-chrome-link` ghost-style "History" link in the brand bar (hidden when already in history). `onHistory` threaded to ResultsView.
- [x] `ResultsView.jsx`: `rv-history-cta` button (shared style with `rv-neuro-cta`), label "Your history", taking the `onHistory` prop.
- Built via /dev-framework-rl episode 01KSAH8EVKKXKC0D7TPSXD9YX4. **Both plan critics FAILed on round 1** (plan-eng FAIL 48 surfaced the raw-payload-projection crit; plan-design FAIL 38 surfaced the empty/loading/error/cap-states all undefined). Revised spec PHASE-D-REVISED.md folds all 15 must_fix items; both critics PASS on retry 1 (eng 86, design 87). code-review FAIL 72 caught a real test bug (`'"heart"'` in forbiddenSubstrings collided with the `kind:'heart'` discriminator, would always fail on correct code) — fixed at root, retry PASS 92. independent-review PASS 92 (4 low nits documented).

What's intentionally NOT in this phase: row tap-to-expand / re-open the past report (deferred); pagination beyond limit=50; server-side filter by `kind`; the `rv-history-cta` getting a dedicated CSS rule (inherits `rv-neuro-cta`). The next plan is Phase B + C (Phase D was prioritised first per the roadmap-batch sequencing because it has the lowest regulatory exposure and gets a piece of the dashboard plan shipped while the MHRA determination is pending).

### Next plan (TBD)

Admin-dashboard Phase B: the privacy-critical aggregate read API with min-N=5 suppression, org-scoped reads, and `requireOrgAdmin` session middleware. **Blocked on the MHRA Class I wellness vs IIa SaMD determination** — Phase B is the surface that actually reads aggregate employee health data, so the regulatory position should be settled before merging.
